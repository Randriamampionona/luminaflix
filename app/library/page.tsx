...
export default async function LibraryPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [t, tFilters] = await Promise.all([getTranslations("pages.library"), getTranslations("filters")]);
  const data = getLibrary(page, sort, genre);
  ...
  const shortcuts: { href: string; label: string; icon: LucideIcon; active: boolean }[] = [
    {
      href: "/library?sort=vote_average.desc",
      label: t("shortcuts.topRated"),
      icon: Star,
      active: sort === "vote_average.desc",
    },
    {
      href: "/library?sort=popularity.desc",
      label: t("shortcuts.popular"),
      icon: TrendingUp,
      active: sort === "popularity.desc",
    },
