...
export default function LuminaAnimePlayer({
  ...
  const handleAdFinished = () => {
    ...
    setIsUnlocked(true); // Unlocks the "Bypass Protocol Ready" screen
    // We do NOT set showTheater(true) here, so it shows your bridge UI first.
  };
    ...
    <div className="w-full space-y-8 animate-in fade-in duration-1000">
      ...
      <div className="flex items-end flex-col space-y-2">
        <div className="relative aspect-video max-h-[60vh] md:max-h-[69vh] w-full overflow-hidden bg-black border border-white/10 shadow-2xl ring-1 ring-white/5">
          ...
          {/* EXTERNAL/BRIDGE THEATER (The actual active Iframe) */}
          {showTheater && (
          ...
          {/* BRIDGE LAUNCHER / PREVIEW PLAYER (Shown after ad, before play) */}
          ...
          {!isUnlocked && !isAdPlaying && (
            ...
            >
              ...
              <p className="mt-8 text-[12px] font-black uppercase italic tracking-[0.5em] text-white/80 group-hover/unlock:text-cyan-400 transition-colors">
        ...
        {(activeTab === "VO" ? VO_PROVIDERS : FR_PROVIDERS).map((provider) => {
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-4 px-6 py-5 rounded-[1.5rem] transition-all duration-500 border ${isActive ? "bg-white border-white shadow-xl" : "bg-zinc-900/40 border-white/5 hover:border-white/20"}`}
            >
              <div
